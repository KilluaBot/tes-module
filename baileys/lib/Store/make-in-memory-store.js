            ...
            if (existsSync(folder)) {
                if (folder.includes('.json')) {
                    ...
                    if (!existsSync(folder + 'chats.json')) {
                        chats = JSON.parse(readFileSync(folder + 'chats.json', { encoding: 'utf-8' }));
                    }
                    let contacts = {};
                    if (!existsSync(folder + 'contacts.json')) {
                        contacts = JSON.parse(readFileSync(folder + 'contacts.json', { encoding: 'utf-8' }));
                    }
                    let messages = {};
                    if (!existsSync(folder + 'messages.json')) {
                        messages = JSON.parse(readFileSync(folder + 'messages.json', { encoding: 'utf-8' }));
                    }
                    fromJSON({
                        chats: chats,
                        contacts: contacts,
